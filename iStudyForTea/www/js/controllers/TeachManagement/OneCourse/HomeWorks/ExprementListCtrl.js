...
app.controller("ExprementListCtrl", function ($scope, $stateParams, $ionicModal, $ionicHistory, $state, $ionicLoading, httpService, subDateToMin) {
    //监听事件 加载菜单栏
    ...
    promise.then(function (data) {
        for (var i = 0; i < data.length; i++) {
            data[i].datestart = subDateToMin.divedeToDay(data[i].datestart);
            data[i].dateend = subDateToMin.divedeToDay(data[i].dateend);
        }
        $scope.items = data;
        $ionicLoading.hide();
