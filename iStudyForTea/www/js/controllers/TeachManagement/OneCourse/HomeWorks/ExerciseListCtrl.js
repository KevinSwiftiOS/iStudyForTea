...
app.controller("ExerciseListCtrl", function ($scope, $stateParams, $ionicModal, $ionicHistory, $state, httpService, $ionicLoading, subDateToMin) {
    //监听事件 加载菜单栏
    ...
    promise.then(function (data) {
        for (var i = 0; i < data.length; i++) {
            data[i].datestart = subDateToMin.divedeToDay(data[i].starttime);
            data[i].dateend = subDateToMin.divedeToDay(data[i].endtime);
        }
        $scope.items = data;
        $ionicLoading.hide();
